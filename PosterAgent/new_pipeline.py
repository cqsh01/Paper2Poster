...
if __name__ == '__main__':
    ...
    explicit_size = args.poster_width_inches is not None and args.poster_height_inches is not None
    if explicit_size:
    ...
    if args.ablation_no_tree_layout:
        ...
        try:
            ...
            print(f"Training layout data is missing ({exc}). Falling back to LLM-only layout.")
            panel_arrangement, figure_arrangement, text_arrangement, input_token, output_token = no_tree_get_layout(
                poster_width,
                poster_height,
                panels,
            ...
            total_input_tokens_t += input_token
            total_output_tokens_t += output_token
            print(f'Fallback layout token consumption: {input_token} -> {output_token}')
            detail_log['fallback_layout_in_t'] = input_token
            detail_log['fallback_layout_out_t'] = output_token
        else:
    ...
    def get_figure_path_by_id(collection, figure_id, path_key):
        if not collection:
            return None
        candidate_keys = [figure_id, str(figure_id)]
        try:
            candidate_keys.append(int(figure_id))
        except (TypeError, ValueError):
            pass
        for key in candidate_keys:
            if key in collection and isinstance(collection[key], dict) and path_key in collection[key]:
                return collection[key][path_key]
        try:
            idx = int(figure_id)
            values = list(collection.values()) if isinstance(collection, dict) else list(collection)
            if 0 <= idx < len(values) and isinstance(values[idx], dict) and path_key in values[idx]:
                return values[idx][path_key]
    ...
    for i in range(len(figure_arrangement)):
        ...
        if 'image' in figure_info:
            ...
            figure_path = get_figure_path_by_id(images, figure_id, 'image_path')
        elif 'table' in figure_info:
            figure_id = figure_info['table']
            figure_path = get_figure_path_by_id(tables, figure_id, 'table_path')
    ...
    safe_model_name = f'{args.model_name_t}_{args.model_name_v}'.replace('/', '_').replace('\\', '_')
    safe_poster_name = args.poster_name.replace('/', '_').replace('\\', '_')
    output_dir = os.path.join('output_posters', safe_model_name, safe_poster_name)
