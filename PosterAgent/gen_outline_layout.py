...
def gen_outline_layout_v2(args, actor_config):
    ...
    def normalize_filtered_information(info):
        ...
        if isinstance(info, list):
            ...
            for idx, item in enumerate(info):
                if not isinstance(item, dict):
                    continue
                item_id = item.get('id') or item.get('index') or item.get('table_id') or item.get('image_id') or idx
                normalized[str(item_id)] = item
            return normalized
        return {}

    filtered_table_information = normalize_filtered_information(filtered_table_information)
    filtered_image_information = normalize_filtered_information(filtered_image_information)
