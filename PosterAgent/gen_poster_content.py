...
def gen_bullet_point_content(args, actor_config, critic_config, agent_modify=True, tmp_dir='tmp'):
    ...
    # Optional visual examples for overflow/blank-space critic.
    # The trimmed Windows Docker version can run without assets/.
    neg_img_path = 'assets/overflow_example_v2/neg.jpg'
    pos_img_path = 'assets/overflow_example_v2/pos.jpg'
    has_overflow_examples = os.path.exists(neg_img_path) and os.path.exists(pos_img_path)
    if not has_overflow_examples:
        print('Overflow example assets are missing; using critic without example images.')
    ...
    def _process_section(i):
        ...
        for j, text_arrangement in enumerate(target_textboxes[:num_textboxes]):
            ...
            while True:
                ...
                if args.model_name_v.startswith('vllm_qwen') or args.ablation_no_example or not has_overflow_examples:
